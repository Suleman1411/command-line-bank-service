...
class Customer {
    ...
    constructor(fName: string, lName: string, age: number, mob: number, acc: number) {
    ...
    transaction(accObj: BankAccount) {
        ...
        if (index !== -1) {
            ...
            if (!this.firstTime) {
                ...
                console.log(
                    `Dear ${chalk.green.italic(customer?.fullName)}, Your Account Number Balance is ${chalk.bold.blueBright("$" + accObj.balance)}`
                );
            }
        }
    }
...
async function bankService(bank: Bank) {
    ...
    if (service.select === "View Balance") {
        ...
        if (account) {
            ...
            console.log(
                `Dear ${chalk.green.italic(customer?.fullName)}, Your Account Number Balance is ${chalk.bold.blueBright("$" + account.balance)}`
            );
        }
    }
